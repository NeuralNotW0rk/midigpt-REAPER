...
# False or 2 (boundary aware) or 2.1 (baseline) model
SECTION_BOUNDARY_EXPERIMENT = 2.1


def process_section_boundaries(s: str):
    if SECTION_BOUNDARY_EXPERIMENT == 2.1:
        kill_strs = set()
        for k, v in enc._INSTRUCTION_TO_MEASUREMENT_DICT.items():
            if v[0] in (
            'current_section_boundary_type', 'next_section_boundary_type', 'measure_countup', 'measure_countdown'):
                kill_strs.add(f';<instruction_{k}>')
        null_instr = enc._MEASUREMENT_TO_INSTRUCTION_DICT[('null_token', 0)]
        null_instr = f';<instruction_{null_instr}>'
        for kill_str in kill_strs:
            s = s.replace(kill_str, null_instr)
    return s
...
def load_models(task):
    ...
    # unjoined model
    model_path = r'models_section_boundary_experiments_2\unjoined\infill\finetuned_epoch_58_0\model'
    model_path = os.path.join('models_section_boundary_experiments_2', 'unjoined', 'infill', 'finetuned_epoch_58_0', 'model')
...
def call_nn_infill(s, S, use_sampling=True, min_length=10, enc_no_repeat_ngram_size=0,
    ...
    t0 = time.time()
    s = process_section_boundaries(s)

    s_request_normalized = normalize_requests(s)

    # print(S)
    # not used below - this just shows that the argument passing works
    ...
    t1 = time.time() - t0
    print('time:', t1, 'sec')
    print('tokens per sec:', this_candidate.count(';')/t1)
    print('notes per sec:', (this_candidate.count(';N') + this_candidate.count(';D')) / t1)
